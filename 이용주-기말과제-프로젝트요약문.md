...
**운영 비용 / 데이터**: 영상 mp4 는 저장 안 함 (YouTube 호스팅). DB 에는 영상 1편당 ~20~55KB (메타+자막+요약). Supabase Free 500MB 안에서 약 1만 편 보관 가능 → 월 100편 처리 시 8~10년 무료 운영. **실제 월 운영비 ≈ $2.5 (OpenAI 만)**, Supabase/Vercel/GitHub 모두 무료 tier.

| 한계 | 향후 개선 |
|---|---|
| 신규 등록 채널의 RSS 15편 이전 영상 백필 불가 (단, 일일 cron 운영 중에는 신규 영상 누락 없음 → 정상 운영에선 한계 아님) | YouTube Data API 도입 시 등록 직후 백필 가능 (쿼터·인증 부담) |
