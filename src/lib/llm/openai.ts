  ...
  schema: {
    ...
    properties: {
      ...
      people: {
        ...
        description:
          '자막/설명에서 명시적으로 언급된 인물만. 추측 금지. 모르면 빈 배열.',
      },
      companies: {
        type: 'array',
        items: { type: 'string' },
...
4. people: "사람"만 포함. 기업·브랜드·제품은 절대 여기 넣지 말 것 (그건 companies 필드로).
   ★ 채울 근거 — [내용](자막)과 [설명문] 둘 다 활용한다. 자막에 게스트 호명이 없어도 [설명문] 상단에 "[○○○ 교수]" 같은 출연자 블록이 있으면 그것이 1차 근거다.
   ★ name: 사람 이름 그대로. 음성 인식 결과를 임의로 보정하지 마라. 단, [설명문]에 표기가 있으면 그 표기를 우선 신뢰한다.
   ...
   ★ 채널 진행자·MC·호스트는 제외. 영상이 게시된 채널의 주인이나 정기 진행자는 출연자가 아니라 진행자이므로 people에 넣지 마라. 예: "권순표의 뉴스하이킥"에서 권순표, "슈카월드"에서 슈카, "김작가 TV"에서 김작가, "신사임당" 채널의 신사임당, "한경 글로벌마켓"의 고정 기자 패널 등은 모두 진행자로 분류. 게스트로 초대된 외부 인물(교수·전문가·CEO 등)만 등재한다. [설명문]에 "출연:" 항목이 명시되어 있으면 그 명단을 1차 근거로 삼되, 그 명단의 인물이 채널 정기 진행자에 해당하면 제외한다.

5. companies: 영상에서 언급된 기업·회사·브랜드 및 연구기관·대학·연구소. [내용]과 [설명문] 모두에서 수집. 영상 전 구간을 빠짐없이 살펴라(후반부도 포함).
...
6. source_type, language: 입력값을 그대로 반환.
