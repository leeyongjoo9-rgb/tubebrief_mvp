"""기말과제 발표자료 .pptx 생성 — 베이지 클래식 디자인.

디자인 시스템:
- 배경: 옅은 웜 베이지 (#F5F0E8) — 매거진/북 디자인 톤
- 카드: 순 흰 배경 + 옅은 보더 + 그림자 (존재감)
- 메인 액센트: 딥 그린 (#2D4A3E) — 영문 카테고리, 강조 (베이지와 클래식 조합)
- 포인트: 와인 (#8B2635) — PROBLEM/CHALLENGE 등 부정 라벨
- 보조 액센트: 오크 골드 (#B5793E) — 성공/긍정/링크
- 텍스트: 짙은 갈색 (#2C2418) + 따뜻한 그레이 (#6B6356)

사용자 주문 반영:
1. 페이지 제목 크기 30% 감소
2. 표지 하단 설명 폰트 키움
3. DEMO 페이지: 메인 박스 하나만 크게 (캡처 자리), 시연 흐름 박스 제거
5. 다크 → 베이지 톤 + 박스 존재감 (보더 + 그림자)
"""
...
# ── 베이지 클래식 팔레트 ─────────────────────────────────
BG_BEIGE = RGBColor(0xF5, 0xF0, 0xE8)         # 슬라이드 배경 — 웜 베이지
CARD_WHITE = RGBColor(0xFF, 0xFF, 0xFF)       # 카드 배경
CARD_BORDER = RGBColor(0xE5, 0xDD, 0xD0)      # 카드 보더 (베이지보다 살짝 진함)
SHADOW = RGBColor(0xD5, 0xC9, 0xB4)            # 그림자 (베이지+그레이)
DIVIDER = RGBColor(0xD5, 0xCC, 0xB8)           # 구분선

TEXT_DARK = RGBColor(0x2C, 0x24, 0x18)         # 본문 — 짙은 따뜻한 갈색
TEXT_GRAY = RGBColor(0x6B, 0x63, 0x56)         # 보조 — 따뜻한 그레이
TEXT_FAINT = RGBColor(0x9B, 0x91, 0x82)        # 가장 흐린 텍스트

ACCENT_GREEN = RGBColor(0x2D, 0x4A, 0x3E)      # 메인 액센트 — 딥 그린
ACCENT_WINE = RGBColor(0x8B, 0x26, 0x35)       # 포인트 — 와인 (부정 라벨)
ACCENT_GOLD = RGBColor(0xB5, 0x79, 0x3E)       # 보조 — 오크 골드 (긍정)

INFO_BG = RGBColor(0xF9, 0xF5, 0xEE)            # 옅은 정보 박스 배경 (베이지보다 살짝 밝음)
...
def add_rounded_rect(slide, x, y, w, h, fill=None, line=None, line_width=0.75,
                     corner_pct=0.04):
...
def add_slide_bg(slide):
    """슬라이드 전체에 옅은 베이지 배경."""
    add_rect(slide, 0, 0, SLIDE_W, SLIDE_H, fill=BG_BEIGE)


def add_card(slide, x, y, w, h, fill=None, with_shadow=True):
    """그림자 + 흰 카드 (존재감 있는 박스).
    그림자: 본 카드 뒤에 살짝 큰 베이지 그레이 사각형 (오프셋)."""
...
def add_slide_header(slide, eng, kor, slide_num, kor_size=22):
    ...
    제목 크기 30% 축소: 32→22, 38→26, 28→19 등.
    """
    # 영문 카테고리 (그린, 작게, letter-spacing 인상)
    ...
    # 헤더 아래 구분선
    add_rect(slide, Inches(0.7), Inches(1.75),
             Inches(11.9), Inches(0.015), fill=DIVIDER)
    # 우하단 슬라이드 번호 (모노 스타일 흉내, 작은 회색)
    add_text(slide, Inches(12.2), Inches(7.05), Inches(0.7), Inches(0.3),
             f"{slide_num:02d} / 09", size=9, color=TEXT_FAINT,
             align=PP_ALIGN.RIGHT)


def add_kicker(slide, x, y, w, h, label, color=None):
    """작은 영문 라벨 (uppercase, letter-spacing 인상)."""
...
def add_icon_card(slide, x, y, w, h, title, body, icon_color=None,
                  title_size=13):
    """존재감 있는 카드 + 좌상단 작은 정사각 아이콘 + 제목 + 본문."""
    icon_color = icon_color or ACCENT_GREEN
    add_card(slide, x, y, w, h)
    # 아이콘 (작은 그린 정사각형)
    add_rect(slide, Inches(x + 0.3), Inches(y + 0.3),
             Inches(0.4), Inches(0.4), fill=icon_color)
    # 제목
    ...
    add_text(slide, Inches(x + 0.3), Inches(y + 0.95), Inches(w - 0.6),
             Inches(h - 1.1), body_items, size=11, color=TEXT_GRAY,
             line_spacing=1.4)


def add_row_card(slide, x, y, w, h, label, content):
    """기술 스택 / 표 행 카드 — 흰 카드 + 좌측 작은 그린 점 + 카테고리 + 내용."""
    add_card(slide, x, y, w, h)
    # 좌측 작은 그린 마커 (작은 정사각형)
    add_rect(slide, Inches(x + 0.3), Inches(y + h / 2 - 0.06),
             Inches(0.12), Inches(0.12), fill=ACCENT_GREEN)
    # 라벨
    add_text(slide, Inches(x + 0.6), Inches(y), Inches(3.0), Inches(h),
             label, size=11.5, bold=True, color=TEXT_DARK,
             anchor=MSO_ANCHOR.MIDDLE)
    # 내용
    add_text(slide, Inches(x + 3.8), Inches(y), Inches(w - 4.0), Inches(h),
             content, size=10.5, color=TEXT_GRAY,
             anchor=MSO_ANCHOR.MIDDLE, line_spacing=1.3)


def add_flow_step(slide, x, y, w, h, title, sub=""):
    """단계 박스 — 흰 카드 + 그린 텍스트 (베이지 톤에 맞춤)."""
    add_card(slide, x, y, w, h)
    ...
    if sub:
        add_text(slide, Inches(x), Inches(y + 0.6), Inches(w), Inches(h - 0.65),
                 sub, size=9, color=TEXT_GRAY, align=PP_ALIGN.CENTER,
...
def add_hyperlink_text(slide, x, y, w, h, text, url, size=11,
...
def main():
    ...
    # 좌상단 강의 라벨 (둥근 보더 칩)
    chip = add_rounded_rect(s, Inches(0.7), Inches(0.7),
                             Inches(3.6), Inches(0.4),
                             fill=CARD_WHITE, line=CARD_BORDER, corner_pct=0.45)
    add_text(s, Inches(0.85), Inches(0.7), Inches(3.4), Inches(0.4),
             "BAVECODING  ·  바이브코딩 기말과제", size=9.5, bold=True,
             color=ACCENT_GREEN, anchor=MSO_ANCHOR.MIDDLE)

    # 브랜드 마크 (작은 그린 박스 + T)
    mark = add_rect(s, Inches(0.7), Inches(2.4), Inches(0.95), Inches(0.95),
                    fill=ACCENT_GREEN)
    add_text(s, Inches(0.7), Inches(2.4), Inches(0.95), Inches(0.95),
             "T", size=32, bold=True, color=CARD_WHITE,
             align=PP_ALIGN.CENTER, anchor=MSO_ANCHOR.MIDDLE)
    add_text(s, Inches(1.8), Inches(2.55), Inches(6), Inches(0.4),
             "CONTENT  DIGEST", size=11, bold=True, color=TEXT_FAINT)

    # 큰 제목 (60 → 42pt, 30% 축소)
    add_text(s, Inches(0.7), Inches(3.55), Inches(11), Inches(1.0),
             "TubeBrief", size=42, bold=True, color=TEXT_DARK)
    # 부제 (그대로 — 사용자가 하단 설명 키우라 한 거지 부제 아님)
    add_text(s, Inches(0.7), Inches(4.55), Inches(11), Inches(0.5),
             "유튜브 구독 채널 자동 요약 서비스",
             size=18, bold=True, color=ACCENT_GREEN)

    # 하단 설명 — 폰트 키움 (14 → 17)
    add_text(s, Inches(0.7), Inches(5.25), Inches(9.0), Inches(1.0),
             "구독한 유튜브 채널의 신규 영상을 매일 자동으로 감지해 AI로 구조화된 "
             "한 장짜리 요약 보고서를 작성·아카이빙하는 개인용 콘텐츠 다이제스트.",
             size=17, color=TEXT_GRAY, line_spacing=1.5)
    ...
    add_text(s, Inches(0.7), Inches(6.75), Inches(2), Inches(0.25),
             "PRESENTER", size=8.5, bold=True, color=TEXT_FAINT)
    ...
    add_text(s, Inches(4.5), Inches(6.75), Inches(2), Inches(0.25),
             "GITHUB", size=8.5, bold=True, color=TEXT_FAINT)
    ...
    add_text(s, Inches(9.0), Inches(6.75), Inches(2), Inches(0.25),
             "SERVICE", size=8.5, bold=True, color=TEXT_FAINT)
    ...
    # 좌측 PROBLEM 카드
    add_card(s, x=0.7, y=2.1, w=5.95, h=4.9)
    add_kicker(s, x=0.95, y=2.35, w=3, h=0.3, label="PROBLEM  ·  문제 · 사용 상황",
               color=ACCENT_WINE)
    add_text(s, Inches(0.95), Inches(2.7), Inches(5.5), Inches(0.45),
             "이런 불편을 매일 겪는다", size=15, bold=True, color=TEXT_DARK)
    add_text(s, Inches(0.95), Inches(3.4), Inches(5.5), Inches(3.4),
             ...
             size=12, color=TEXT_GRAY, line_spacing=1.6)

    # 우측 GOAL 카드
    add_card(s, x=6.85, y=2.1, w=5.95, h=4.9)
    add_kicker(s, x=7.1, y=2.35, w=3, h=0.3, label="GOAL  ·  대상 사용자 · 해결 가설")
    add_text(s, Inches(7.1), Inches(2.7), Inches(5.5), Inches(0.45),
             "이런 사용자를 위해", size=15, bold=True, color=TEXT_DARK)
    add_text(s, Inches(7.1), Inches(3.4), Inches(5.5), Inches(2.0),
             ["•  채널을 등록만 해두면 매일 자동으로 요약 보고서 누적",
              "•  영상 한 편을 1분 안에 파악 → 골라서 시청",
              "•  단순 캡션 X → 헤드라인·본문·출연자·기업·시각 점프까지 구조화"],
             size=12, color=TEXT_GRAY, line_spacing=1.6)
    # 사용자 정의 박스 (강조)
    add_rect(s, Inches(7.1), Inches(5.55), Inches(5.55), Inches(1.3),
             fill=INFO_BG, line=CARD_BORDER, line_width=0.5)
    add_text(s, Inches(7.3), Inches(5.7), Inches(5.2), Inches(1.0),
             ["▸  구독 채널 3개 이상, 콘텐츠 중심 시청 / 시간 부족한 개인",
              "▸  정보 비용 최소화로 의사결정에 쓰고 싶은 사용자"],
             size=11.5, color=TEXT_DARK, line_spacing=1.55, bold=True)
    ...
    for i, (title, body) in enumerate(cards):
        ...
        x = 0.7 + col * 4.10
        y = 2.05 + row * 2.40
        add_icon_card(s, x=x, y=y, w=3.90, h=2.15, title=title, body=body,
                      title_size=12.5)
    ...
    # INPUT
    add_kicker(s, x=0.7, y=2.2, w=3, h=0.3, label="INPUT  ·  입력 (1회 등록)",
               color=ACCENT_WINE)
    add_text(s, Inches(0.7), Inches(2.55), Inches(11.9), Inches(0.6),
             "사용자는 메인화면 [채널 관리] 에서 채널 URL + 키워드 필터를 1회 등록한다.",
             size=13, color=TEXT_DARK, line_spacing=1.4)

    # AUTO 흐름
    add_kicker(s, x=0.7, y=3.55, w=5, h=0.3,
    ...
    # OUTPUT
    add_kicker(s, x=0.7, y=5.4, w=3, h=0.3, label="OUTPUT  ·  결과")
    add_card(s, x=0.7, y=5.75, w=5.9, h=1.5)
    add_text(s, Inches(0.95), Inches(5.95), Inches(5.4), Inches(1.15),
             ["▸  홈 대시보드 — ChannelStrip, 카드 그리드, 마지막 자동 확인 시각",
              "▸  채널별 필터링, 최신 영상이 좌측 첫 카드부터 배치"],
             size=11.5, color=TEXT_DARK, line_spacing=1.5)
    add_card(s, x=6.85, y=5.75, w=5.9, h=1.5)
    add_text(s, Inches(7.1), Inches(5.95), Inches(5.4), Inches(1.15),
             ["▸  영상 상세 — 헤드라인 + 키워드 + 출연자 / 거론 인물",
              "▸  본문 안 (mm:ss) → 유튜브 그 시점 점프. 한 번 본 보고서는 삭제"],
             size=11.5, color=TEXT_DARK, line_spacing=1.5)

    # ────────────────────────────────────────────
    # Slide 5 — DEMO · 데모 화면 (큰 박스 1개)
    # ────────────────────────────────────────────
    s = make_slide(prs)
    add_slide_bg(s)
    ...
    # 라이브 URL (작게, 하이퍼링크)
    add_kicker(s, x=0.7, y=2.1, w=3, h=0.3, label="LIVE", color=ACCENT_GOLD)
    add_hyperlink_text(s, Inches(1.3), Inches(2.1), Inches(8), Inches(0.3),
                       "https://tubebrief-mvp.vercel.app/",
                       "https://tubebrief-mvp.vercel.app/",
                       size=11.5, bold=True)
    add_text(s, Inches(0.7), Inches(2.5), Inches(11.9), Inches(0.4),
             "URL 클릭 → 라이브 사이트. 접속 불가 시 다음 슬라이드의 캡처 본 페이지로 이동.",
             size=10, color=TEXT_FAINT)

    # 큰 메인 화면 박스 1개 (캡처 자리)
    add_card(s, x=0.7, y=3.05, w=11.9, h=4.0)
    # 안내 텍스트
    add_text(s, Inches(0.7), Inches(4.7), Inches(11.9), Inches(0.4),
             "[ 메인 화면 캡처 — 홈 대시보드 ]",
             size=14, bold=True, color=TEXT_FAINT,
             align=PP_ALIGN.CENTER, anchor=MSO_ANCHOR.MIDDLE)
    add_text(s, Inches(0.7), Inches(5.1), Inches(11.9), Inches(0.35),
             "헤더 + ChannelStrip + 카드 그리드 (55개 요약 영상)",
             size=10.5, color=TEXT_FAINT,
             align=PP_ALIGN.CENTER, anchor=MSO_ANCHOR.MIDDLE)
    ...
    # 파이프라인 4단계
    add_kicker(s, x=0.7, y=2.1, w=5, h=0.3, label="PIPELINE  ·  4단계 직렬 처리")
    ...
    for i, (t, sub) in enumerate(pipeline):
        ...
        if i < len(pipeline) - 1:
            add_arrow(s, cur_x + step_w + 0.04, 2.85)
        cur_x += step_w + 0.15
    ...
    for i, (label, content) in enumerate(rows):
        y = 4.05 + i * 0.78
        add_row_card(s, x=0.7, y=y, w=11.9, h=0.68,
    ...
    rows = [
        ...
        ("Vercel Platform (인프라)",
         "vercel.json crons → 매일 0시 /api/cron. push 시 GitHub webhook 자동 재배포"),
    ]
    for i, (label, content) in enumerate(rows):
        y = 2.2 + i * 0.90
        add_row_card(s, x=0.7, y=y, w=11.9, h=0.78,
                     label=label, content=content)
    # 한계
    ...
    for i, (label, content) in enumerate(rows):
        ...
        add_row_card(s, x=0.7, y=y, w=11.9, h=0.78,
                     label=label, content=content)
    # 입력 자료
    add_kicker(s, x=0.7, y=5.95, w=5, h=0.3,
               label="INPUT  ·  Context 작성을 위한 자료")
    add_text(s, Inches(0.7), Inches(6.3), Inches(11.9), Inches(1.0),
             ...
             size=10, color=TEXT_GRAY, line_spacing=1.45)
    ...
    # 좌측: 어려움 + 해결
    add_card(s, x=0.7, y=2.1, w=6.1, h=4.2)
    add_kicker(s, x=0.95, y=2.3, w=4, h=0.3, label="CHALLENGE  ·  어려움",
               color=ACCENT_WINE)
    add_text(s, Inches(0.95), Inches(2.65), Inches(5.8), Inches(0.5),
             "LLM 한국어 요약이 규칙을 안 지킴", size=14, bold=True, color=TEXT_DARK)
    add_text(s, Inches(0.95), Inches(3.2), Inches(5.8), Inches(0.85),
             ["•  초기 gpt-4o-mini: brief 290자 / 1문단 / (mm:ss) 0개",
              "•  프롬프트 '가이드' → LLM 이 권고로 받아들임"],
             size=11, color=TEXT_GRAY, line_spacing=1.4)
    add_kicker(s, x=0.95, y=4.25, w=4, h=0.3, label="SOLUTION  ·  3겹 가드레일")
    add_text(s, Inches(0.95), Inches(4.6), Inches(5.8), Inches(1.55),
    ...
    # 우측: 결과 비교 표
    add_card(s, x=7.0, y=2.1, w=5.7, h=4.2)
    add_kicker(s, x=7.25, y=2.3, w=4, h=0.3,
    ...
    add_text(s, Inches(7.4), Inches(th_y), Inches(1.9), Inches(0.4),
             "항목", size=10.5, bold=True, color=TEXT_DARK,
             anchor=MSO_ANCHOR.MIDDLE)
    add_text(s, Inches(9.3), Inches(th_y), Inches(1.5), Inches(0.4),
             "옛 규칙", size=10.5, bold=True, color=TEXT_GRAY,
             anchor=MSO_ANCHOR.MIDDLE, align=PP_ALIGN.CENTER)
    add_text(s, Inches(10.8), Inches(th_y), Inches(1.6), Inches(0.4),
             "새 규칙", size=10.5, bold=True, color=ACCENT_GREEN,
             anchor=MSO_ANCHOR.MIDDLE, align=PP_ALIGN.CENTER)
    table_rows = [
    ...
    for i, (a, b, c) in enumerate(table_rows):
        ry = th_y + 0.45 + i * 0.5
        add_text(s, Inches(7.4), Inches(ry), Inches(1.9), Inches(0.45),
                 a, size=11, color=TEXT_DARK, anchor=MSO_ANCHOR.MIDDLE)
        add_text(s, Inches(9.3), Inches(ry), Inches(1.5), Inches(0.45),
                 b, size=11, color=TEXT_GRAY, anchor=MSO_ANCHOR.MIDDLE,
                 align=PP_ALIGN.CENTER)
        add_text(s, Inches(10.8), Inches(ry), Inches(1.6), Inches(0.45),
                 c, size=11, bold=True, color=ACCENT_GREEN,
                 anchor=MSO_ANCHOR.MIDDLE, align=PP_ALIGN.CENTER)

    # 하단: 한계 (전체 폭)
    add_kicker(s, x=0.7, y=6.45, w=4, h=0.3,
               label="LIMITATION  ·  정직한 회고", color=ACCENT_WINE)
    add_text(s, Inches(0.7), Inches(6.8), Inches(12.0), Inches(0.55),
             ["•  Vercel Hobby 60초 / 일 1회 cron — backlog → Pro 또는 외부 워커 분리      "
              "•  자막 음역 (곽재식→곽제식) → description 해시태그를 LLM 정답 표기로 주입      "
              "•  배포 URL 공개 → Vercel Password Protection 또는 Supabase Auth"],
             size=9.5, color=TEXT_GRAY, line_spacing=1.45)
